...
Static reference evidence owner: Assistant / repository source.
Live local-site evidence owner: User.
...
## 4. Corrected R5-E6-A visual evidence ownership

The approved static reference is stored in GitHub and is directly readable by the assistant. The assistant is responsible for rendering and inspecting it at the required viewport sizes.

The user must not recreate or upload static-reference screenshots.

### Assistant-generated static evidence
...
### User-provided live local-site evidence
...
The local URL `http://spatialflow.local/cart-2/` is not accessible from the assistant runtime, so only the three live screenshots must come from the user.

Required conditions for the live screenshots:

```text
browser page zoom: 100%
exact viewport size shown above
page fully loaded
no stale cache
no Console test styles
no inspector highlighting or red rectangles
no open modal, menu, Coupon or validation state
same Cart item quantity/data state across A2, A4 and A6
...
```text
Assistant: render and inspect A1, A3 and A5 from the GitHub static source.
User: return only A2, A4 and A6 from the local live Cart.
```

No source edit or final page-status decision is authorized before the live screenshots are reviewed.